		...
		public function widget($args, $instance)
		{
			...
			if( is_iterable( $taxonomies ) ):
				...
				foreach( $taxonomies as $tx ):
					if( isset($selects) && is_array($selects) && in_array($tx->attribute_id, $selects)){
						...
						foreach($terms as $term){
							...
							if($tx->attribute_type == 'image'){
								...
								echo '<span class="label"><span class="mfn_attr_icon" style="background-image: url('.wp_get_attachment_image_src($value, 'thumbnail')[0].');"><input type="checkbox" '.($active == 1 ? "checked" : null).' name="mfn_'.$term->taxonomy.'[]" id="mfn_attr_'.$term->term_id.'" value="'.$term->slug.'"></span></span>';
							}else if($tx->attribute_type == 'color'){
								$value = $mfn_value = get_term_meta($term->term_id, 'mfn_attr_field', true);
								echo '<span class="label"><span class="mfn_attr_icon" style="background-color: '.$value.';"><input type="checkbox" '.($active == 1 ? "checked" : null).' name="mfn_'.$term->taxonomy.'[]" id="mfn_attr_'.$term->term_id.'" value="'.$term->slug.'"></span></span>';
							}else if($tx->attribute_type == 'label'){
								echo '<span class="label"><span><input type="checkbox" '.($active == 1 ? "checked" : null).' name="mfn_'.$term->taxonomy.'[]" id="mfn_attr_'.$term->term_id.'" value="'.$term->slug.'"></span>'.$term->name.'</span>';
							}else{
								echo '<span class="label"><span class="mfn_attr_icon"><input type="checkbox" '.($active == 1 ? "checked" : null).' name="mfn_'.$term->taxonomy.'[]" id="mfn_attr_'.$term->term_id.'" value="'.$term->slug.'"> '.$term->name.'</span></span>';
							}
				...
				endforeach;
				echo '<button type="submit" class="button mfn-btn">' . esc_html( 'Filter', 'woocommerce' ) . '</button>';
				echo '</form>';
			endif;
