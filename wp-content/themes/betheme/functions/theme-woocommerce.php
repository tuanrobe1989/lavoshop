...
if (! function_exists('mfn_woo_styles')) {
	function mfn_woo_styles()
	{
        ...
        array(
          ...
          'productthumbsover' => mfn_opts_get('shop-product-gallery-overlay') ? mfn_opts_get('shop-product-gallery-overlay') : 0,
          'productthumbs' => mfn_opts_get('shop-product-thumbnails-margin') ? mfn_opts_get('shop-product-thumbnails-margin') : 0,
          'mainimgmargin' => mfn_opts_get('shop-product-main-image-margin') ? mfn_opts_get('shop-product-main-image-margin') : 0,
        )
	    );
...
function woo_template_body_classes( $classes ) {
  ...
  if ( is_product() ) {
	...
	if(mfn_opts_get('shop-wishlist-position') == 0){
...
function mfn_append_excerpt_loop(){
	...
	if( mfn_opts_get( 'shop-excerpt' ) ){
		echo '<div class="excerpt">'. apply_filters( 'woocommerce_short_description', get_the_excerpt( $product->get_id() ) ) .'</div>';
	}
}
...
function mfn_append_wishlist_button(){
	echo '<a href="#" data-id="'.get_the_ID().'" class="mfn-wish-button tooltip tooltip-txt" data-tooltip="Add to wishlist"><i class="icon-heart-empty-fa"></i><i class="icon-heart-fa"></i></a>';
}
					...
					case 'label':
						echo '<ul class="mfn-vr-options mfn-vr-labels" data-atr="'.$atr_slug.'">';
							foreach($atr as $item){
								...
								echo '<li><a href="'.get_the_permalink($product->get_id()).'?'.$a.'='.$atr_item->slug.'" data-id="'.$atr_item->slug.'">'.$atr_item->name.'</a></li>';
								}
							}
						echo '</ul>';
						break;
					case 'color':
						echo '<ul class="mfn-vr-options mfn-vr-color" data-atr="'.$atr_slug.'">';
							foreach($atr as $item){
								...
								echo '<li class="tooltip tooltip-txt" data-tooltip="'.$atr_item->name.'"><a href="'.get_the_permalink($product->get_id()).'?'.$a.'='.$atr_item->slug.'" data-id="'.$atr_item->slug.'"><span style="background-color: '.$mfn_value.';"></span></a></li>';
								}
							}
						echo '</ul>';
						break;
					case 'image':
						echo '<ul class="mfn-vr-options mfn-vr-image" data-atr="'.$atr_slug.'">';
							foreach($atr as $item){
								...
								if(isset($atr_item->name)){
								$mfn_value = get_term_meta($atr_item->term_id, 'mfn_attr_field', true);
								echo '<li class="tooltip tooltip-txt" data-tooltip="'.$atr_item->name.'"><a href="'.get_the_permalink($product->get_id()).'?'.$a.'='.$atr_item->slug.'" data-id="'.$atr_item->slug.'">'.wp_get_attachment_image($mfn_value, 'thumbnail').'</a></li>';
								}
							}
						echo '</ul>';
						break;
						...
						echo '<select class="mfn-vr-select" data-atr="'.$atr_slug.'">';
							...
							foreach($atr as $item){
								...
								if(isset($atr_item->slug)){
									echo '<option data-link="'.get_the_permalink($product->get_id()).'?'.$a.'='.$atr_item->slug.'" value="'.$atr_item->slug.'">'.$atr_item->name.'</option>';
								}else{
									echo '<option data-link="'.get_the_permalink($product->get_id()).'?'.$a.'='.trim($item).'" value="'.trim($item).'">'.trim($item).'</option>';
								}
							}
						echo '</select>';
						break;
	...
	if(count($attr_taxonomies) > 0){
		foreach($attr_taxonomies as $attr){
			if( in_array($attr->attribute_type, array('color', 'image') )){
				add_action( 'pa_'.$attr->attribute_name.'_edit_form_fields', 'mfn_edit_tax_form_fields' );
				add_action( 'pa_'.$attr->attribute_name.'_add_form_fields', 'mfn_edit_tax_form_fields' );
...
function mfn_edit_tax_form_fields ($tag) {
...
if (! function_exists('mfn_woo_per_page')) {
	function mfn_woo_per_page($cols)
	{
		...
		if(isset($tmp_id) && is_numeric($tmp_id) && get_post_status($tmp_id) == 'publish' && get_post_type($tmp_id) == 'template' && null !== get_post_meta($tmp_id, 'mfn_template_perpage', true) ){
			// if is template
