								...
								<?php if( isset($conditions) && count($conditions) > 0){ $x = 0; foreach($conditions as $c=>$cond){ ?>
									...
									<div class="df-row-inputs">
										...
										<select name="mfn_template_conditions[<?php echo $x; ?>][producttag]" class="mfn-form-control df-input df-input-opt df-input-producttag <?php if($cond->var == 'producttag') {echo 'show';} ?>">
											...
											<option <?php if($cond->var != 'shop' && $cond->producttag == $cat->term_id){ echo 'selected'; } ?> value="<?php echo $tag->term_id ?>"><?php echo $tag->name; ?></option>
											<?php } endif; ?>
										</select>
									</div>
		  						...
		  						if(isset($products) && count($products) > 0){
		  							foreach ($products as $product) {
		  								if($con->rule == 'include'){
		  									...
		  									$checkifexists = get_post_meta( $product->ID, 'mfn_product_cat_template', true );
		  									if( !isset($checkifexists) || empty($checkifexists) ){
		  										update_post_meta( $product->ID, 'mfn_product_cat_template', 'excluded' );
		  									}
		  								}
		  							}
		  						}
				  						...
				  						if(isset($products) && count($products) > 0){
				  							foreach ($products as $product) {
				  								if($con->rule == 'include'){
				  									...
				  									$checkifexists = get_post_meta( $product->ID, 'mfn_product_tag_template', true );
				  									if( !isset($checkifexists) || empty($checkifexists) ){
				  										update_post_meta( $product->ID, 'mfn_product_tag_template', 'excluded' );
				  									}
				  								}
				  							}
				  						}
