    ...
    public function show( $items = false ){
    	...
    	if( get_post_meta($this->post_id, 'mfn-page-local-style') ){
  		...
  		if( isset( $items ) && is_array( $items ) ){
  			// for ajax
  			$mfn_items = $items;
  		} else if( ( empty( $_GET['preview'] ) && empty( $_GET['mfn-preview'] ) ) || ( !empty( $_GET['visual'] ) && $_GET['visual'] == 'iframe' ) || ( !empty($_GET['preview']) && $_GET['preview'] = 'true' ) ){
  			// empty preview i mfn-preview
  			// or is visual=iframe or is preview=true
