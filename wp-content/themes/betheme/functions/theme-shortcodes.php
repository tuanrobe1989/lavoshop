...
if (! function_exists('sc_product_short_description')) {
	function sc_product_short_description($attr, $product = false)
	{
		...
		//woocommerce_template_single_excerpt( $product->get_id() );
		$output .= get_the_excerpt( $product->get_id() );
		$output .= '</div>';
		return $output;
	}
}
...
if (! function_exists('sc_shop_products')) {
	function sc_shop_products($attr, $sample = false){
		...
		$classes[] = 'columns-'. $columns[$attr['layout']];
		$classes[] = $attr['layout'];

		// isotope

		if( 'masonry' == $attr['layout'] ){
			$classes[] = 'isotope';
		};
		...
		$output  .= '<ul class="products '.implode(' ', $classes).'">';
			while ( have_posts() ) {
				the_post();
				global $product;
				if($attr['layout'] == 'masonry') $classes = array( 'isotope-item' );
		...
		elseif( $sample ):
			...
			$output .= '<div class="woocommerce-info alert alert_info"><div class="alert_icon"><i class="icon-help"></i></div><div class="alert_wrapper">'.__('No products were found matching your selection.', 'woocommerce').'</div></div>';
		endif;
