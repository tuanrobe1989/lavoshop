  ...
  $(function($) {
    ...
    function init(){
      ...
      $(group).not('.pill-initialized').each(function(){
        ...
        $(this).addClass('pill-initialized');
    ...
    function PillField(domLocation){
      ...
      this.checkForExistingClassess = function() {
        ...
        if(  that.hiddenInputDOM.val().lenght ){
      ...
      this.createPillsHTML = function(text){
        ...
        if($content.find(that.element).length){
          $content.find(that.element).addClass(text);
        }
      ...
      this.pushWordToArray = function() {
        ...
        //input not empty and not duplicate of other pill
        if( cleanedText.length > 0 &&  !_.contains(that.pills, cleanedText) ){
