...
if( get_post_type($post->ID) == 'template' && get_post_meta($post->ID, 'mfn_template_type', true) != 'default' ){
	$shop_items = get_post_meta($post->ID, 'mfn_template_type', true);
}
...
echo '<div class="panel panel-items" id="mfn-widgets-list">
		...
		foreach($widgets as $w=>$widget){
			if( (!isset($shop_items) && !in_array($widget['cat'], array('shop-archive', 'single-product')) ) || ( isset($shop_items) && $shop_items == 'single-product' && $widget['cat'] != 'shop-archive' ) || ( isset($shop_items) && $shop_items == 'shop-archive' && $widget['cat'] != 'single-product' ) ){
